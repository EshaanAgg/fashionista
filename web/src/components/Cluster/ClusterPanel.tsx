...
const TuneableOptions = ["clothesCutoff", "accessoriesCutoff", "weightName", "weightColor", "weightFeatures"]
...
export function ClusterPanel() {
  ...
  return <Panel position="top-right"><Box maw={600} className='p-2 m-2 bg-white'>
    {
      (Object.keys(values) as GraphOptionsKey[]).filter((key) => TuneableOptions.includes(key)).map((key) => (
        <div key={key}>
          <Text size="sm"> {key} </Text>
          <Slider value={values[key]} onChange={(newValue) => {
            setValues({
              ...values,
              [key]: newValue
            })
